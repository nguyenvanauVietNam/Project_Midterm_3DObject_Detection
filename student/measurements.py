...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        ############
        if self.name == 'lidar':
            # For lidar, the field of view is defined as a cone. Check if the object is within this cone.
            theta = np.arctan2(x[1], x[0])
            return self.fov[0] <= theta <= self.fov[1]
        elif self.name == 'camera':
            # For camera, we need to check if the object is within the field of view angles.
            angle_x = np.arctan2(x[0], x[2])
            angle_y = np.arctan2(x[1], x[2])
            return (self.fov[0] <= angle_x <= self.fov[1]) and (self.fov[0] <= angle_y <= self.fov[1])
        return False
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            pos_veh = np.ones((4, 1)) # homogeneous coordinates
            pos_veh[0:3] = x[0:3] 
            pos_cam = self.veh_to_sens * pos_veh # transform from vehicle to camera coordinates
            
            # Project to image coordinates
            z_cam = pos_cam[2, 0]
            if z_cam == 0:
                raise ValueError("Projection error: z coordinate is zero")
            
            x_img = (self.f_i * pos_cam[0, 0] / z_cam) + self.c_i############
            # TODO Step 4: implement nonlinear camera measurement function h:
            # - transform position estimate from vehicle to camera coordinates
            # - project from camera to image coordinates
            # - make sure to not divide by zero, raise an error if needed
            ...
            def get_hx(self, x):    
                ...
                if self.name == 'lidar':
                    ...
                    x_img = (self.f_i * pos_cam[0, 0] / z_cam) + self.c_i
                    y_img = (self.f_j * pos_cam[1, 0] / z_cam) + self.c_j
            ...
            def get_H(self, x):
                ...
                if self.name == 'lidar':
                    # For lidar, the Jacobian is simply the identity matrix
                    H = np.eye(3)
                elif self.name == 'camera':
                    # For camera, we need to compute the Jacobian of the projection
                    pos_veh = np.ones((4, 1)) # homogeneous coordinates
                    pos_veh[0:3] = x[0:3] 
                    pos_cam = self.veh_to_sens * pos_veh # transform from vehicle to camera coordinates
                    ...
                    H = np.array([
                        [self.f_i / z_cam, 0, -self.f_i * pos_cam[0, 0] / (z_cam**2)],
                        [0, self.f_j / z_cam, -self.f_j * pos_cam[1, 0] / (z_cam**2)]
                    ])
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.z[1] = z[1]
            self.sensor = sensor # sensor that generated this measurement
            sigma_camera_x = params.sigma_camera_x # load params
            sigma_camera_y = params.sigma_camera_y
            self.R = np.matrix([[sigma_camera_x**2, 0], # measurement noise covariance matrix
                                [0, sigma_camera_y**2]])
        
            # Note: camera measurements do not include width, length, height, and yaw
        ############
        # END student code
        ############
